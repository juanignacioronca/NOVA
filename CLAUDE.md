...
- **Fase:** Prompt 10 — Reconocimiento de personas + cierre ✅ **COMPLETO**. 🎉 **ROADMAP TERMINADO.** NOVA reconoce **quién** se acerca (cara) y **quién** habla (voz) por embeddings locales enrolados en la memoria, y el proyecto queda cerrado y operable (smoke e2e + `HANDOFF.md` + prompts de auditoría). Después del cierre se hizo un **ajuste fino** (sección siguiente).

### Construido (Ajuste fino post-roadmap)
- [x] **Lista de compras** (`tools/compras.py`): `agregar_compra` (low) / `ver_compras` (safe) / `quitar_compra` (low) sobre `data/compras.json`. `parsear_pedido()` interpreta lenguaje natural ("agregá pan a la lista", "sacá la leche de la lista", "¿qué hay que comprar?") y lo rutean el Conductor (`_tool_por_intencion`) y `respuestas_rapidas` (intent `shopping`). Allowlist + permisos actualizados.
- [x] **Avisos proactivos del calendario + qué llevar:** `agendar_evento` acepta `llevar` (o lo **sugiere** por actividad: playa → protector solar/toalla, tenis → raqueta/pelotas, camping, trekking, etc. — `sugerir_llevar`). `parse_cuando` convierte `cuando` (ISO, `12/6 17:00`, "hoy/mañana a las HH") a timestamp y `proximos()` filtra la ventana; el `ProactiveScheduler` avisa los eventos que arrancan dentro de `event_horizon_hours` (config `perception.yaml → proactive`, default 2 h) **con los elementos a llevar**.
- [x] **Especialistas por área (sub-agentes reales):** `teams.yaml` suma **inversiones_research** (groq), **inversiones_riesgo** (deepseek), **inversiones_operador** (local), **fitness_nutricionista**, **fitness_coach**, **ecommerce_marketing**, **ecommerce_operaciones**. El coordinador hace que el líder de área **consulte primero a su propio equipo** (`_especialistas`: intersección de su `puede_consultar` con los miembros) y después a los transversales. `max_inter_agent_hops` 3 → **8** (el tope de gasto sigue siendo `max_model_calls=20`). Claves nuevas `area_investigador`/`area_razonador` en `models.yaml` siguiendo la regla de modelos.
- [x] **Contexto por persona reconocida:** `recognition/presencia.py → contexto_de()` jala de la memoria **pendientes + eventos + preferencias** (ej. plan nutricional) de la persona matcheada; queda en el evento de presencia y en `WorldState["persona_presente"]`. El Conductor lo suma al recall del turno (paso 1d) → si NOVA te reconoce a vos tiene **tu** contexto disponible; si reconoce a tu papá, el de él.
- [x] **Tests offline (75/75):** ciclo completo de compras por el Conductor, parseo NL, `parse_cuando`/sugerencias/ventana, aviso proactivo de evento con "llevar" (sin repetir), especialistas en el roster + líder consulta a su equipo, contexto por persona en presencia y en el recall del Conductor.
...
- **Topes por petición** (`empresa.yaml`): `max_subtareas` (corta la descomposición), `max_inter_agent_hops` (corta las consultas entre agentes; subió a 8 con los especialistas por área), `max_model_calls` (corta el gasto de modelo). Al alcanzar un tope, se corta con aviso y se integra lo hecho. Controla costo (clave con free-tiers) y es una barrera de seguridad.
